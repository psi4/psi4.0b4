...
#define DEBUGINTS 0
    ...
    timer_on("Hess: ECP");
    {
        ...
        const double* buffer = ecpint->buffer();
...
#define DEBUGINTS 0
                ...
                while(ecpint->next_hessian_ecp()) {
                    ...
                    const double *AxAx = buffer +  0*offset;
                    const double *AxAy = buffer +  1*offset;
                    const double *AxAz = buffer +  2*offset;
                    const double *AyAy = buffer +  3*offset;
                    const double *AyAz = buffer +  4*offset;
                    const double *AzAz = buffer +  5*offset;
                    const double *AxBx = buffer +  6*offset;
                    const double *AxBy = buffer +  7*offset;
                    const double *AxBz = buffer +  8*offset;
                    const double *AyBx = buffer +  9*offset;
                    const double *AyBy = buffer + 10*offset;
                    const double *AyBz = buffer + 11*offset;
                    const double *AzBx = buffer + 12*offset;
                    const double *AzBy = buffer + 13*offset;
                    const double *AzBz = buffer + 14*offset;
                    const double *AxCx = buffer + 15*offset;
                    const double *AxCy = buffer + 16*offset;
                    const double *AxCz = buffer + 17*offset;
                    const double *AyCx = buffer + 18*offset;
                    const double *AyCy = buffer + 19*offset;
                    const double *AyCz = buffer + 20*offset;
                    const double *AzCx = buffer + 21*offset;
                    const double *AzCy = buffer + 22*offset;
                    const double *AzCz = buffer + 23*offset;
                    const double *BxBx = buffer + 24*offset;
                    const double *BxBy = buffer + 25*offset;
                    const double *BxBz = buffer + 26*offset;
                    const double *ByBy = buffer + 27*offset;
                    const double *ByBz = buffer + 28*offset;
                    const double *BzBz = buffer + 29*offset;
                    const double *BxCx = buffer + 30*offset;
                    const double *BxCy = buffer + 31*offset;
                    const double *BxCz = buffer + 32*offset;
                    const double *ByCx = buffer + 33*offset;
                    const double *ByCy = buffer + 34*offset;
                    const double *ByCz = buffer + 35*offset;
                    const double *BzCx = buffer + 36*offset;
                    const double *BzCy = buffer + 37*offset;
                    const double *BzCz = buffer + 38*offset;
                    const double *CxCx = buffer + 39*offset;
                    const double *CxCy = buffer + 40*offset;
                    const double *CxCz = buffer + 41*offset;
                    const double *CyCy = buffer + 42*offset;
                    const double *CyCz = buffer + 43*offset;
                    const double *CzCz = buffer + 44*offset;
